...
// 投稿の一覧検索
func SerchPostALL() ([]model.Post, error) {
	// SQLを定義
	...
	for rows.Next() {
		var post model.Post
		err := rows.Scan(&post.ID, &post.Content, &post.UserID, &post.CreateAT, &post.UpdateAT)
		if err != nil {
			// エラーログの出力
...
func SearchPost(id int) (model.Post, error) {
	...
	err := row.Scan(&post.ID, &post.Content, &post.UserID, &post.CreateAT, &post.UpdateAT)
	if err != nil {
		// エラーログの出力
		log.Printf("投稿の詳細検索に失敗しました: %v", err)
		return model.Post{}, fmt.Errorf("投稿の詳細検索に失敗しました: %w", err)
	}
...
func UpdatePost(id int, content string, createdUserID int) (int64, error) {
	...
	if err != nil {
		// エラーログの出力
		log.Printf("更新された行数の取得に失敗しました: %v", err)
		return 0, fmt.Errorf("更新された行数の取得に失敗しました: %w", err)
	}
...
func DeletePost(id int) (int64, error) {
	...
	if err != nil {
		...
		return 0, fmt.Errorf("削除された行数の取得に失敗しました: %w", err)
	}
